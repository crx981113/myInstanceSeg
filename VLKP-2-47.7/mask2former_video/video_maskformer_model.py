...
class VideoMaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        """
        images = []
        for video in batched_inputs:  # 在images列表中添加单张图片
            for frame in video["image"]:
                images.append(frame.to(self.device))
        images = [(x - self.pixel_mean) / self.pixel_std for x in images] # 图片归一化操作
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_video in targets:
            ...
            for f_i, targets_per_frame in enumerate(targets_per_video["instances"]):
                ...
                if isinstance(targets_per_frame.gt_masks.tensor, np.ndarray):
                    targets_per_frame.gt_masks.tensor = torch.tensor(targets_per_frame.gt_masks.tensor,
                                                                     dtype=torch.bool)
